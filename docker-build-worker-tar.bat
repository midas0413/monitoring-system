@echo off
REM Docker Worker 이미지 빌드 및 tar 파일 저장 스크립트 (Windows)
...
set IMAGE_NAME=%IMAGE_PREFIX%-worker
set TAR_FILE=%IMAGE_NAME%-%VERSION%.tar

echo ==========================================
echo Building monitoring-worker Docker image...
echo ==========================================
echo.

echo Building worker image...
docker build -f Dockerfile.worker -t %IMAGE_NAME%:%VERSION% .
...
echo Saving worker image to %TAR_FILE%...
